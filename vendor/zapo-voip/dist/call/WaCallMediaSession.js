...
const srtcp_js_1 = require("../crypto/srtcp.js");
...
const pcm_frame_accumulator_js_1 = require("../media/pcm-frame-accumulator.js");
...
const signaling_diagnostics_js_1 = require("../signaling/signaling-diagnostics.js");
...
class WaCallMediaSession {
    ...
    srtcpSendCount = 0;
    opusCodec = null;
    voipSettings = (0, voip_settings_js_1.parseVoipSettings)(undefined);
    sctpRelay;
    audioEngine;
    selfSsrc = 0;
    selfStreamSsrcs = [];
    ...
    inboundPcmFrames = new pcm_frame_accumulator_js_1.PcmFrameAccumulator(INBOUND_PCM_FRAME_SAMPLES);
    relayPacketCount = 0;
    stunResponseCount = 0;
    selfEchoCount = 0;
    lastRecvSeq = -1;
    ...
    async initMedia(selfLid, peerJid) {
        ...
        this.logger.debug('voip_diag media_identity_initialized', {
            ...
            selfMediaJid: selfDeviceJid,
            peerMediaJid: (0, ssrc_js_1.formatE2ESrtpParticipantId)(peerJid),
        ...
        this.opusCodec = await mlow_codec_js_1.MLowCodec.create(this.codecOptions);
    }
    async configureVoipSettings(settings, source, force = false) {
        ...
        if (changed && this.opusCodec) {
            ...
            replacement = await mlow_codec_js_1.MLowCodec.create(this.codecOptionsFor(settings));
        }
    ...
    async prepareIncomingRelay() {
        if (this.info.direction !== types_js_1.CallDirection.Incoming ||
            this.info.isEnded ||
    ...
    async sendIncomingPreaccept(peerJid) {
        try {
            const preacceptNode = (0, signaling_js_1.buildIncomingPreacceptStanza)(peerJid, this.info.callId, this.info.callCreator, this.signalingAudioRate);
    ...
    async handleCallAccept(node, peerJid) {
        ...
        const ourBaseJids = new Set([credentials?.meLid, credentials?.meJid]
            .filter((jid) => !!jid)
            .map((jid) => (0, protocol_1.toUserJid)(jid)));
        const acceptingBaseJid = acceptingDeviceJid
            ? (0, protocol_1.toUserJid)(acceptingDeviceJid)
            : '';
        ...
        let peerCallKey;
        if ((0, signaling_js_1.needsDecryption)(nodeInfo.tag)) {
            try {
                const decryptedPeerCallKey = await (0, signaling_js_1.decryptCallKey)(this.deps, nodeInfo.innerNode, peerJid, this.logger.child({ component: 'signaling' }));
                const ourCallKey = this.info.encryptionKey;
                if (decryptedPeerCallKey &&
                    ourCallKey &&
                    !(0, util_1.uint8TimingSafeEqual)(ourCallKey, decryptedPeerCallKey)) {
    ...
    async handleCallPreaccept(node, peerJid) {
        ...
        for (const endpoint of this.info.relayData.endpoints) {
            ...
            try {
                const relayLatencyNode = (0, signaling_js_1.buildRelayLatencyStanza)(this.info.peerJid, callId, callCreator, [
                    {
                        relayName,
        ...
        try {
            const transportNode = (0, signaling_js_1.buildTransportStanza)((0, protocol_1.toUserJid)(peerJid), callId, callCreator, meId);
            await this.deps.lowLevelCoordinator.sendNode(transportNode);
            this.logger.debug('voip_diag outgoing_preaccept_signaling_sent', {
                callId,
                peerJid: (0, protocol_1.toUserJid)(peerJid),
    ...
    async handleCallTransport(_node) {
        ...
        if (!nodeInfo)
            return;
        const nestedRelays = this.info.direction === types_js_1.CallDirection.Incoming
            ? this.ingestIncomingRelayUpdate(_node, 'transport')
            : [];
        const relays = nestedRelays.length > 0
            ? nestedRelays
            : (0, signaling_js_1.extractRelayEndpoints)(nodeInfo.innerNode);
        if (relays.length > 0 && !this.sctpRelay.hasConnection()) {
            ...
            if (this.info.direction === types_js_1.CallDirection.Incoming) {
                if (this.info.stateData.state !== types_js_1.CallState.IncomingRinging) {
                    await this.prepareIncomingRelay();
                }
                return;
            }
    ...
    async handleCallAck(node) {
        ...
        const ackVoipSettings = (0, voip_settings_js_1.parseVoipSettingsFromNode)(node);
        ...
        if (relays.length > 0) {
            ...
            if (this.info.isInitiator && !this.outgoingPreacceptSent) {
                ...
                try {
                    const preacceptNode = (0, signaling_js_1.buildOutgoingPreacceptStanza)(this.info.peerJid, this.info.callId, this.info.callCreator, this.signalingAudioRate);
                    await this.deps.lowLevelCoordinator.sendNode(preacceptNode);
    ...
    async handleCallRelaylatency(node, peerJid) {
        const nodeInfo = (0, signaling_js_1.extractNodeInfo)(node);
        if (!nodeInfo || this.info.direction !== types_js_1.CallDirection.Incoming)
            return;
        this.ingestIncomingRelayUpdate(node, 'relaylatency');
        const relaySignaling = (0, relay_diagnostics_js_1.summarizeRelaySignaling)(node);
        ...
        for (const te of teNodes) {
            ...
            try {
                const response = (0, signaling_js_1.buildRelayLatencyStanza)(peerJid, callId, callCreator, [
                    {
                        relayName,
                        latency,
                ...
                this.logger.debug('voip_diag outbound_relaylatency_response', {
                    ...
                    envelope: (0, signaling_diagnostics_js_1.summarizeCallEnvelope)(response)
                });
                responded++;
            }
            catch (err) {
    ...
    ingestIncomingRelayUpdate(node, source) {
        const relaySignaling = (0, relay_diagnostics_js_1.summarizeRelaySignaling)(node);
        const parsed = (0, relay_ack_js_1.parseRelayFromAck)(node);
    ...
    async handleCallMuteV2(node, peerJid) {
        ...
        const meId = credentials?.meLid || credentials?.meJid || '';
        try {
            const muteNode = (0, signaling_js_1.buildMuteV2Stanza)(peerJid, this.info.callId, this.info.callCreator, 0, meId);
    ...
    async sendOutgoingPostAcceptSignaling(acceptingDeviceJid) {
        if (this.info.direction !== types_js_1.CallDirection.Outgoing ||
    ...
    initSrtpKeys(receiveCallKey) {
        ...
        const ourDeviceJid = this.selfMediaJid || (0, ssrc_js_1.formatE2ESrtpParticipantId)(ourCredJid);
        ...
        try {
            ...
            const receiveKeyings = peerKeyJids.map((jid) => (0, encryption_js_1.derivePerJidSrtpKey)(receiveCallKey || callKey, jid));
            ...
            this.srtcpSender = new srtcp_js_1.SrtcpSender(sendKeying, this.selfSsrc);
    ...
    async sendPendingAccept() {
        if (!this.acceptPending || this.acceptSent)
            return;
        const acceptStanza = (0, signaling_js_1.buildDirectAcceptStanza)(this.info.callId, this.acceptedByJid || this.info.peerJid, this.acceptCallCreator || this.info.callCreator, this.info.mediaType === types_js_1.CallMediaType.Video, this.signalingAudioRate);
        try {
            ...
            this.logger.debug('voip_diag inbound_accept_sent', {
                ...
                selfPid: this.info.relayData?.selfPid,
                peerPid: this.info.relayData?.peerPid,
                envelope: (0, signaling_diagnostics_js_1.summarizeCallEnvelope)(acceptStanza)
            });
            this.armRemoteMediaWatchdog();
        }
        catch (err) {
    ...
    async connectRelays(endpoints) {
        ...
        const candidates = (0, relay_endpoints_js_1.orderMediaRelayCandidates)(endpoints, this.info.direction === types_js_1.CallDirection.Incoming);
    ...
    isCallAcceptedForMedia() {
        ...
        if (state !== types_js_1.CallState.Connecting && state !== types_js_1.CallState.Active)
            return false;
        return this.info.direction === types_js_1.CallDirection.Incoming
            ? this.acceptSent
            : this.acceptedByJid !== null;
    }
    ...
    startSrtcpReports() {
        ...
        this.srtcpTimer = setInterval(() => {
            ...
            try {
                ...
                this.sctpRelay.broadcast((0, bytes_js_1.toArrayBuffer)(packet));
                this.srtcpSendCount++;
