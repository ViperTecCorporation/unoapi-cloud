...
exports.SrtpError = exports.SrtpSession = exports.SrtpContext = void 0;
...
class SrtpContext {
    ...
    constructor(keying, authTagLen) {
        ...
        this.sessionSalt = deriveKey(keying.masterKey, keying.masterSalt, types_js_1.SRTP_LABEL.SALT, 14);
    }
    ...
    protect(packet) {
        ...
        const encrypted = (0, primitives_js_1.aesCtr128)(this.sessionKey, iv, packet.payload);
    ...
    unprotect(data) {
        ...
        const header = rtp_js_1.RtpHeader.decode(data);
        ...
        if (this.authTagLen > 0) {
            ...
            if (!(0, util_1.uint8TimingSafeEqual)(expected, received)) {
                throw new SrtpError('auth_failed', 'SRTP auth tag verification failed');
            }
        }
        const iv = this.generateIv(header.ssrc, index);
        const decrypted = (0, primitives_js_1.aesCtr128)(this.sessionKey, iv, data.subarray(headerSize, headerSize + payloadLen));
        this.advanceReplay(index, estimatedRoc, seq);
        return new rtp_js_1.RtpPacket(header, decrypted);
    }
    updateRoc(seq) {
        if (!this.initialized) {
            this.lastSeq = seq;
    ...
    generateIv(ssrc, index) {
        ...
        (0, bytes_js_1.writeUInt32BE)(this.ssrcBuffer, ssrc, 0);
        for (let i = 0; i < 4; i++) {
            this.ivBuffer[4 + i] ^= this.ssrcBuffer[i];
        }
        (0, bytes_js_1.writeBigUInt64BE)(this.indexBuffer, index, 0);
        for (let i = 0; i < 6; i++) {
            this.ivBuffer[8 + i] ^= this.indexBuffer[2 + i];
        }
        return this.ivBuffer;
    }
    computeAuthTag(data, roc, tagLen = types_js_1.SRTP_AUTH_TAG_LEN) {
        (0, bytes_js_1.writeUInt32BE)(this.rocBuffer, roc, 0);
        const result = (0, primitives_js_1.hmacSha1)(this.authKey, data, this.rocBuffer);
