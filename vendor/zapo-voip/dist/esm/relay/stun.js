...
export function buildAllocateForRelay(relayToken, streamDescriptors, hmacKey, relayIp, relayPort, transactionId = generateTransactionId()) {
    const parts = [];
    ...
    if (relayIp && relayPort) {
        parts.push(encodeAttribute(ATTR_XOR_RELAYED_ADDRESS, encodeXorRelayedAddress(relayIp, relayPort, transactionId)));
    }
 ...
 * is left unanswered.
 */
export function buildBindingSuccessForRequest(request, integrityKey) {
    ...
    if (messageType !== STUN_BINDING_REQUEST)
        return undefined;
    if (readUInt32BE(request, 4) !== STUN_MAGIC_COOKIE)
        return undefined;
...
export function isRtpPacket(data) {
    if (data.length < 2)
        return false;
...
export function parseStunResponse(data) {
    if (data.length < 20)
        return null;
...
export function formatStunResponse(info) {
...
export function classifyPacket(data) {
    if (data.length < 2)
