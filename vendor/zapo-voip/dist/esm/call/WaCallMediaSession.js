...
export class WaCallMediaSession {
    ...
    async initMedia(selfLid, peerJid) {
        ...
        this.selfStreamSsrcs = prepareWasmRelayStreamSsrcs(derivedStreamSsrcs, generateSecureSsrc(this.info.callId, selfDeviceJid, 6));
        const peerSsrc = generateSecureSsrc(this.info.callId, formatE2ESrtpParticipantId(peerJid));
    ...
    onRelayData(data, connectionId) {
        ...
        if (!this.srtpSession || !this.opusCodec)
            return;
        if (!isWhatsappOpusPayloadType(pt))
            return;
        if (data.length >= 12) {
