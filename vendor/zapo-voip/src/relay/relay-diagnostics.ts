import { type BinaryNode, getNodeChildren } from 'zapo-js/transport'
...
export interface RelayAddressSummary {
    ...
    parseOutcome:
        | 'parsed_ipv4'
        | 'parsed_ipv6'
        | 'ignored_missing_address'
        | 'ignored_short_address'
        | 'ignored_unsupported_address_length'
}
...
function contentBytes(content: BinaryNode['content']): number {
...
function summarizeAddress(content: BinaryNode['content']): RelayAddressSummary {
...
export function summarizeRelaySignaling(root: BinaryNode): RelaySignalingSummary {
    const descendants = getDescendantNodes(root)
    const probes: RelayProbeSummary[] = []
    const candidates: RelayCandidateSummary[] = []
    ...
    for (const node of descendants) {
        if (node.tag === 'relay') relayNodeCount++
        if (node.tag === 'participant') participantNodeCount++
