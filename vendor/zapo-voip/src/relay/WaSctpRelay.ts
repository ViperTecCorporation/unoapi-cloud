...
function describeAllocateWire(packet: Uint8Array): Readonly<Record<string, unknown>> {
    ...
    return {
        ...
        endpointAttributeHex: endpoint ? bytesToHex(endpoint.data) : undefined
    }
}

export interface WaSctpRelayOptions {
    readonly logger?: Logger
    readonly nativeTransportFactory?: (
        options: ConstructorParameters<typeof NativeRelayTransport>[0]
    ) => NativeRelayTransport
}
...
export class WaSctpRelay extends EventEmitter {
    ...
    private readonly nativeTransportFactory: NonNullable<
        WaSctpRelayOptions['nativeTransportFactory']
    >
    ...
    private selfPid = 0
    private peerPid = 0
    private readyConnectionIds = new Set<string>()
    private mediaConnectionId: string | null = null
    private mediaConnectionConfirmed = false
    private mediaPendingSelectionDropCount = 0
    private startupMediaFanoutEnabled = false
    private startupMediaFanoutLogCount = 0
    ...
    getReadyConnectionCount(): number {
        ...
        for (const connectionId of this.readyConnectionIds) {
            ...
            if (conn && this.isConnOpen(conn)) count++
        }
        return count
    }
    ...
    selectMediaConnection(connectionId: string, confirmed = false): boolean {
        const conn = this.connections.get(connectionId)
        if (!conn || !this.isConnOpen(conn)) return false
        if (this.mediaConnectionConfirmed && this.mediaConnectionId !== connectionId) return false
        ...
        if (confirmed) this.startupMediaFanoutEnabled = false
    ...
    selectMediaConnectionByRelayId(relayId: number): boolean {
        if (!Number.isSafeInteger(relayId) || relayId < 0) return false
    ...
    waitForReadyCount(minimumCount = 1, timeoutMs = 2000): Promise<number> {
        ...
        return new Promise((resolve) => {
            ...
            const onReady = () => {
                ...
                if (readyCount >= requiredCount) finish(readyCount)
            }
    ...
    async connectToRelay(relayInfo: RelayInfo): Promise<Connection | null> {
        ...
        try {
            ...
            transport.on('message', (buffer: Uint8Array) => {
                if (
                    this.connections.get(connectionId) !== conn ||
            ...
            transport.on('transport_error', (err: Error) => {
    ...
    private sendStunAllocateOnOpen(conn: Connection, relayInfo: RelayInfo): void {
        ...
        if (!conn.cachedAllocate) {
            ...
            conn.cachedAllocate = buildAllocateForRelay(
                relayInfo.rawToken,
                streamDescriptors,
            ...
            this.logger.debug('voip_diag relay_allocate_wire', {
                connectionId,
                relayAddressFamily: isIP(relayInfo.ip),
        ...
        this.logger.debug('voip_diag relay_allocate_sent', {
            ...
            subscriptionSsrcs: this.subscriptionSsrcs.map((ssrc) =>
                `0x${ssrc.toString(16).padStart(8, '0')}`
            ),
            selfPid: this.selfPid,
            peerPid: this.peerPid,
    ...
    private drainBuffer(connectionId: string): void {
        const conn = this.connections.get(connectionId)
        if (!conn || !this.isConnOpen(conn)) return
    ...
    private rtcpRecvCount = 0
    private remoteRtpRecvCount = 0
    private remoteRtcpRecvCount = 0
    ...
    private handleRelayMessage(data: Uint8Array, relayInfo: RelayInfo, conn: Connection): void {
        ...
        if (isRtp && !conn.hasReceivedFirstRtp) {
            ...
            this.logger.debug('voip_diag first_relay_rtp_on_connection', {
                ...
                addressFamily: isIP(relayInfo.ip),
                tokenId: relayInfo.tokenId,
                authTokenId: relayInfo.authTokenId,
                sequence: data.length >= 4 ? (data[2] << 8) | data[3] : undefined,
                ssrc:
                    data.length >= 12
                        ? `0x${readUInt32BE(data, 8).toString(16).padStart(8, '0')}`
                        : undefined
            })
        }
        ...
        if (twoBits === 0) {
            ...
            if (stunInfo) {
                ...
                if (stunInfo.rawType === 0x0001 && relayInfo.key) {
                    const bindingSuccess = buildBindingSuccessForRequest(
                        data,
                        TEXT_ENCODER.encode(relayInfo.key)
                    )
                ...
                if (stunInfo.method === 'wa-pong') {
                    ...
                    if (stunInfo.isError) {
                        ...
                        if (
                            ...
                            this.logger.warn(
                                'ipv6 relay allocation rejected; quarantining candidate',
                                {
                                    connectionId: conn.id,
                                    relayName: relayInfo.name,
                                    relayId: relayInfo.relayId,
        ...
        if (isMedia) {
            ...
            if (this.startupMediaFanoutEnabled && !this.mediaConnectionConfirmed) {
                ...
                if (sent > 0) {
                    ...
                    if (
                        this.startupMediaFanoutLogCount === 1 ||
            ...
            if (this.relayMap.size > 1) {
                ...
                if (
                    this.mediaPendingSelectionDropCount === 1 ||
     ...
     */
    resetTransport(reason = 'relay_reconfigure'): void {
