...
export async function buildOfferStanza(
    ...
    const explicitDevices = [...new Set((peerDevices ?? []).map((jid) => jid.trim()))].filter(
        (jid) => jid.length > 0 && toUserJid(jid) === peerUserJid
    )
    ...
    const devices = explicitDevices.length
        ? explicitDevices
        : (await deps.signalDeviceSync.syncDeviceList([peerJid])).flatMap(
              (entry) => entry.deviceJids
          )
 ...
 */
export function buildDirectAcceptStanza(
    callId: string,
    peerJid: string,
    callCreator: string,
    isVideo: boolean,
    audioRate: '8000' | '16000' = '16000'
): BinaryNode {
    const acceptContent: BinaryNode[] = [
...
function buildPreacceptStanzaWithCapability(
    peerJid: string,
    callId: string,
    callCreator: string,
    capability: Uint8Array,
    audioRate: '8000' | '16000'
): BinaryNode {
    return {
        tag: 'call',
        attrs: { to: peerJid, id: generateCallStanzaId() },
...
export function buildIncomingPreacceptStanza(
    peerJid: string,
    callId: string,
    callCreator: string,
    audioRate: '8000' | '16000' = '16000'
): BinaryNode {
    return buildPreacceptStanzaWithCapability(
        peerJid,
        callId,
        callCreator,
        CAPABILITY_INCOMING_PREACCEPT,
        audioRate
    )
}

/** ViperConnect caller-side capability preserved from the live outbound success. */
export function buildOutgoingPreacceptStanza(
    peerJid: string,
    callId: string,
    callCreator: string,
    audioRate: '8000' | '16000' = '16000'
