...
import { isLegacySession, isOnlineStatus, sessionLabel, sessionPhone } from '../domain/session.js?v=4.0.0-beta8';
import { renderContactCards, renderGroupCards } from '../features/entities.js?v=4.0.0-beta8';
import { renderSessionConfig } from '../features/session_config.js?v=4.0.0-beta8';
import { renderWebhooks } from '../features/webhooks.js?v=4.0.0-beta8';
const tabs = [
...
const renderOverview = (session) => `
  <section class="stats" aria-label="Resumo da sessão">
    ...
    <article class="stat-card"><span>Contatos</span><strong>Por sessão</strong><small>diretório Zapo paginado</small></article>
  ...
  <section class="section">
    ...
    <div class="action-grid">
      ...
      <button class="action-card action-card--danger" type="button" data-action="deregister-session" data-phone="${escapeHtml(sessionPhone(session))}">${icon('trash')}<span><strong>Deregister</strong><small>Remover vínculo e exigir novo pareamento</small></span></button>
    </div>
  </section>
`;
const renderContacts = (contacts, hasMore, loading, error) => `
  <section class="section">
    ...
    ${loading ? '<div class="loading-state">Carregando contatos…</div>' : renderContactCards(contacts)}
    ${hasMore && !loading ? `<div class="load-more"><button class="btn" type="button" data-action="load-more-contacts">Carregar mais</button></div>` : ''}
  </section>
`;
const renderGroups = (groups, loading, error) => `
  <section class="section">
    <div class="section__heading">
    ...
    ${error ? `<div class="inline-error">${escapeHtml(error)}</div>` : ''}
    ${loading ? '<div class="loading-state">Carregando grupos…</div>' : renderGroupCards(groups)}
  </section>
`;
const renderPanel = (options) => {
    ...
    if (options.tab === 'contacts') {
        return renderContacts(options.contacts, options.contactsHasMore, options.loadingSection, options.sectionError);
    }
    if (options.tab === 'webhooks')
        return `<section class="section">${renderWebhooks(options.session.webhooks || [])}</section>`;
    if (options.tab === 'groups')
        return renderGroups(options.groups, options.loadingSection, options.sectionError);
    return renderOverview(options.session);
};
