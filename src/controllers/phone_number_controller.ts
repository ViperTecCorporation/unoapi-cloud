...
export class PhoneNumberController {
  ...
  public async get(req: Request, res: Response) {
    ...
    try {
      ...
      if (fields) {
        ...
        for (const f of fields.split(',').map((v) => v.trim()).filter(Boolean)) {
          if (Object.prototype.hasOwnProperty.call(graphPhone, f)) selected[f] = (graphPhone as any)[f]
        }
  ...
  public async whatsappBusinessAccounts(req: Request, res: Response) {
    ...
    try {
      ...
      return res.status(200).json({ data: Array.from(ids).map((id) => ({ id })) })
    } catch (e) {
      return sendGraphError(res, 500, e.message, { code: 131016, type: 'GraphMethodException' })
    }
  }
