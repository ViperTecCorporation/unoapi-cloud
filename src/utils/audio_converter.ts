...
import { CONVERT_AUDIO_FFMPEG_PARAMS, SEND_AUDIO_WAVEFORM, WEBHOOK_TIMEOUT_MS, AUDIO_WAVEFORM_SAMPLES } from '../defaults'
...
export default async function (url: string): Promise<{ buffer: Buffer; waveform: Uint8Array | undefined }> {
  ...
  return new Promise<{ buffer: Buffer; waveform: Uint8Array | undefined }>(async (resolve, reject) => {
    const ff = await spawn('ffmpeg', ['-y', '-i', inputFile, ...CONVERT_AUDIO_FFMPEG_PARAMS, outputFile], {
...
async function getAudioWaveform(buffer: Buffer) {
  try {
    ...
    const samples = AUDIO_WAVEFORM_SAMPLES
