...
const renderOverview = (session: SessionConfig): string => `
...
const renderGroups = (groups: GroupSummary[], loading: boolean, error: string): string => `
  <section class="section">
    <div class="section__heading">
...
const renderPanel = (options: SessionPageOptions): string => {
  ...
  if (options.tab === 'contacts') {
    return renderContacts(
      options.contacts,
      options.contactsHasMore,
      options.loadingSection,
      options.sectionError,
    )
  }
  if (options.tab === 'webhooks') return `<section class="section">${renderWebhooks(options.session.webhooks || [])}</section>`
  if (options.tab === 'groups') return renderGroups(options.groups, options.loadingSection, options.sectionError)
