...
const renderRouting = (state: VoipBootstrap) => resourceGrid(state, 'lineGroups', [
  ['Grupo', item => `<strong>${escapeHtml(item.label || item.id)}</strong>`],
  ['Empresa', item => escapeHtml(`${companyName(state, item.companyId)}`)],
...
const field = (name: string, label: string, value: unknown = '', type = 'text', required = false) => `<label class="field"><span>${escapeHtml(label)}</span><input name="${name}" type="${type}" value="${escapeHtml(`${value ?? ''}`)}" ${required ? 'required' : ''}></label>`
...
export const renderVoipResourceModal = (state: VoipBootstrap, resource: VoipResourceName, id = '') => {
  ...
  if (resource === 'companies') fields += `${field('label', 'Nome da empresa', item.label, 'text', true)}${field('timeZone', 'Fuso horário', item.timeZone || 'America/Cuiaba', 'text', true)}`
  ...
  if (resource === 'extensionGroups') fields += `${field('label', 'Nome do grupo', item.label, 'text', true)}${selectField('companyId', 'Empresa', companies, item.companyId, true)}${selectField('extensionIds', 'Ramais', extensions, item.extensionIds, false, true)}`
