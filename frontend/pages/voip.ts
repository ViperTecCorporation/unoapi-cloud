...
import type { VoipBootstrap } from '../domain/types.js'

const resourceEditor = (label: string, resource: string, items: Array<Record<string, any>> = []) => {
  const values = Array.isArray(items) ? items : []
  const editor = (item: Record<string, any>, isNew = false) => `
...
const settingsEditor = (label: string, path: string, value: Record<string, any> = {}) => `
...
export const renderVoipPage = (state: VoipBootstrap, loading: boolean, error = ''): string => {
  ...
  return `
    ...
    <section class="section"><div class="section__heading"><div><h2>${t('Configuração avançada')}</h2><p class="muted">${t('Edição integral dos recursos do serviço VoIP.')}</p></div></div>
      ...
      ${resourceEditor(t('Sessões'), 'sessions', state.sessions as any)}
      ${resourceEditor(t('Ramais'), 'extensions', state.extensions as any)}
